...
def f(xyz, rrr):
	...
	for j in range(top.ns):
		...
		if CorrectionMode == 1:
			derivs.append(speciesq[j]/jperev*(efieldz + (speciesbeta[j]*clight*hl_pth_bar[0,j]/2 - speciesq[j]*xyz[j+top.ns]**2*bfieldz/4/speciesCombined[j].mass)*dBdz) )
		
		if CorrectionMode == 2:
			derivs.append(speciesq[j]/jperev*(efieldz - xyz[j+top.ns]**2/4*d2Edz2 + (speciesbeta[j]*clight*hl_pth_bar[0,j]/2 - speciesq[j]*xyz[j+top.ns]**2*bfieldz/4/speciesCombined[j].mass)*dBdz) )
	...
	for j in range(top.ns):
		...
		emitterm = ((top.hepsr[0,0,j]/2)**2 + (hl_pthn_bar[0,j] /speciesbeta[j])**2) / xyz[j+top.ns]**3
...
for iii in range(0, top.jhist+1):
	...
	pthetalist = array([hl_pthn_bar[iii,j] for j in range(top.ns)])
